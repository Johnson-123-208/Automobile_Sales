...
import { Car, Wrench, Plus, Edit, Trash2, Menu, X, Package, Search, Heart, ArrowLeft } from 'lucide-react';
      ...
      <aside className={`fixed top-0 left-0 h-full w-64 bg-slate-800 border-r border-slate-700 z-40 transform transition-transform duration-300 ${
        ...
        <nav className="p-4 space-y-2">
          <Link
            ...
            className="flex items-center space-x-3 p-3 text-slate-300 hover:bg-slate-700 rounded-lg transition-colors"
          >
            <Package size={20} />
            <span>Dashboard</span>
          </Link>
        ...
        <header className="bg-slate-800 border-b border-slate-700 p-4 lg:p-6">
          <div className="flex items-center justify-between">
            ...
            <button 
              ...
              className="px-4 py-2 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white rounded-xl transition-all duration-300 flex items-center space-x-2"
            >
              <Plus size={20} />
          ...
          <div className="mb-6">
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-3 bg-slate-800 border border-slate-700 rounded-xl text-white placeholder-slate-400 focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 transition-all duration-300"
              />
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredVehicles.map((vehicle) => (
                <motion.div
                  ...
                  className="bg-slate-800 rounded-xl p-6 border border-slate-700 hover:border-indigo-500/50 transition-all duration-300 cursor-pointer"
                >
              ...
              {filteredSpareParts.map((part) => (
                <motion.div
                  ...
                  onClick={() => {
                    ...
                    setSelectedProductType('spare_part');
                    setIsModalOpen(true);
                  }}
                  className="bg-slate-800 rounded-xl p-6 border border-slate-700 hover:border-indigo-500/50 transition-all duration-300 cursor-pointer"
                >
